...
class WebService {
    ...
    entityToString(str) {
        var self = this;
        for (var i in self.entityTable) {
            if (i != 38) {
                str = str.replace(new RegExp(self.entityTable[i], "g"), String.fromCharCode(i));
            }
        }
        str = str.replace(new RegExp("&#(x?)(\\d+);", "g"), function (match, p1, p2, string) {
            return String.fromCharCode(((p1 == 'x') ? parseInt(p2, 16) : p2));
        });
        str = str.replace(new RegExp(self.entityTable[38], "g"), String.fromCharCode(38));
        return str;
    }

    // MARK: ********** String **********

    getContents(str, startStr, endStr) {
        var startStrPos = str.indexOf(startStr);
        var endStrPos = str.indexOf(endStr, startStrPos + 1);
        var toStrs = [];
        while(startStrPos !== -1 && endStrPos !== -1) {
            var toStr = str.substring(startStrPos, endStrPos).replace(startStr, "").replace(endStr, "");
            startStrPos = str.indexOf(startStr, startStrPos + 1);
            endStrPos = str.indexOf(endStr, endStrPos + 1);
            toStrs.push(toStr);
        }
        return toStrs;
    }

    getContent(str, startStr, endStr) {
        var startStrPos = str.indexOf(startStr);
        var endStrPos = str.indexOf(endStr, startStrPos + 1);
        var toStr;
        if (startStrPos !== -1 && endStrPos !== -1) {
            toStr = str.substring(startStrPos, endStrPos).replace(startStr, "").replace(endStr, "");
        }
        return toStr;
    }
